...
//  cfg.mag_dec                   = 0;          // For example, -6deg 37min, = -637 Japan, format is [sign]dddmm (degreesminutes) default is zero.
    ...
    // Autostart
    cfg.as_lnchr                  = 200;        // [50 - 250 no dimension DEFAULT:200] Autostart launchrate to get off the ground. When as_stdev is exceeded, as_clbr takes over
    cfg.as_clbr                   = 100;        // [50 - 250cm/s DEFAULT:100] Autostart climbrate in cm/s after liftoff! Autostart Rate in cm/s will be lowered when approaching targethight.
    cfg.as_trgt                   = 0;          // [0 - 15m  DEFAULT:0 (0 = Disable)] Autostart Targethight in m Note: use 3m or more
    cfg.as_stdev                  = 10;         // [10 - 50 no dimension DEFAULT:10] This is the std. deviation of the variometer when a liftoff is assumed. "10" is most sensitive.
